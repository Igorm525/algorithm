...
var cart = {
  ...
  methods: {
    add: function add(item) {
      ...
      var existed = this.products.find(function (el) {
        return el.id_product === item.id_product;
      });

      if (existed) {
        this.$parent.fetch("cart/".concat(existed.id_product), function (data) {
          if (data.result === 1) {
            existed.quantity++;
        ...
        this.$parent.fetch('cart', function (data) {
          if (data.result === 1) {
    ...
    remove: function remove(item) {
      ...
      this.$parent.fetch("cart/".concat(item.id_product), function (data) {
        if (data.result === 1) {
          if (item.quantity > 1) {
            item.quantity--;
          } else {
  ...
  methods: {
    fetch: function (_fetch) {
      ...
      return fetch(url, _objectSpread({
        ...
        return _this.error.errors.messages.push(error.toString());
      });
    })
  }
};
...
var product = {
  ...
  created: function created() {
    ...
    this.$parent.fetch('products', function (data) {
      _this.products = _this.filtered = data;
    });
  },
